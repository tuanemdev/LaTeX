...
/**
 * An index that points to a particular character in the MathAtomList. The index is a LinkedList that represents
 * a path from the beginning of the MathAtomList to reach a particular atom in the list. The next node of the path
 ...
 */
public class MathAtomListIndex {
     ...
     */
    public enum MathAtomListSubIndexType: Int {
    ...
    var subIndexType: MathAtomListSubIndexType = .none
    
    /// The index into the sublist.
    var subIndex: MathAtomListIndex?
    ...
    func prevIndex() -> MathAtomListIndex? {
        if self.subIndexType == .none {
            if self.atomIndex > 0 {
                return MathAtomListIndex(level0Index: self.atomIndex - 1)
            }
        } else {
            if let prevSubIndex = self.subIndex?.prevIndex() {
                return MathAtomListIndex(at: self.atomIndex, with: prevSubIndex, type: self.subIndexType)
            }
        }
        return nil
    }
    ...
    func nextIndex() -> MathAtomListIndex {
        if self.subIndexType == .none {
            return MathAtomListIndex(level0Index: self.atomIndex + 1)
        } else if self.subIndexType == .nucleus {
            return MathAtomListIndex(at: self.atomIndex + 1, with: self.subIndex, type: self.subIndexType)
        } else {
            return MathAtomListIndex(at: self.atomIndex, with: self.subIndex?.nextIndex(), type: self.subIndexType)
        }
    }
    
    /**
     * Returns true if this index represents the beginning of a line. Note there may be multiple lines in a MathAtomList,
    ...
    func isAtSameLevel(with index: MathAtomListIndex?) -> Bool {
    ...
    func finalSubIndexType() -> MathAtomListSubIndexType {
    ...
    func hasSubIndex(ofType type: MathAtomListSubIndexType) -> Bool {
    ...
    func levelUp(with subIndex: MathAtomListIndex?, type: MathAtomListSubIndexType) -> MathAtomListIndex {
        if self.subIndexType == .none {
            return MathAtomListIndex(at: self.atomIndex, with: subIndex, type: type)
        }
        
        return MathAtomListIndex(at: self.atomIndex, with: self.subIndex?.levelUp(with: subIndex, type: type), type: self.subIndexType)
    }
    
    func levelDown() -> MathAtomListIndex? {
        if self.subIndexType == .none {
            return nil
        }
        
        if let subIndexDown = self.subIndex?.levelDown() {
            return MathAtomListIndex(at: self.atomIndex, with: subIndexDown, type: self.subIndexType)
        } else {
            return MathAtomListIndex(level0Index: self.atomIndex)
        }
    }
    
    /** Factory function to create a `MathAtomListIndex` with no subindexes.
     @param index The index of the atom that the `MathAtomListIndex` points at.
     */
    public init(level0Index: Int) {
        self.atomIndex = level0Index
    }
    
    public convenience init(at location: Int, with subIndex: MathAtomListIndex?, type: MathAtomListSubIndexType) {
...
extension MathAtomListIndex: CustomStringConvertible {
...
extension MathAtomListIndex: Hashable {
...
extension MathAtomListIndex: Equatable {
    public static func ==(lhs: MathAtomListIndex, rhs: MathAtomListIndex) -> Bool {
