...
private class BundleManager {
    ...
    static nonisolated(unsafe) internal let manager = BundleManager()
    ...
    private func registerCGFont(mathFont: MathFont) throws {
        guard let resourceBundleURL = Bundle.module.path(forResource: mathFont.rawValue, ofType: "otf") else {
    ...
    private func registerMathTable(mathFont: MathFont) throws {
        guard let mathTablePlist = Bundle.module.url(forResource: mathFont.rawValue, withExtension: "plist") else {
            throw FontError.fontPathNotFound
        }
